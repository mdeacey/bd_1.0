...
;(function(win, doc, undefined) {
  ...
  win.vUnit = function (options) {
    ...
    var viewportHasChanged = function() {
      /* First check if a valid element ID has been fed in via the options */
      if(vunit.options.containerID !== null && document.getElementById(vunit.options.containerID) !== null)
        var currentViewportSize = calculateContainerSize(vunit.options.containerID);
      ///* else, fall back to using viewport size */
      else currentViewportSize = calculateViewportSize();
     ...
     *
     * @returns {Object} An object containing the dimensions of the container passed in via opts.constainerID.
     *
     * Example:
     * return {
		 *     width: 768,
    ...
    var calculateContainerSize = function(id) {
      var containerSize = {
